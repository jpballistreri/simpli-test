...
exports.update = (req, res) => {
  ...
  Lead.update(req.body, { where: { id: vehicle_id } })
    .then((num) => {
      console.log(num);
      num[0] === 1
        ? res.send({ message: `Lead id:${dealer_id} updated` })
...
exports.delete = (req, res) => {
  ...
  Lead.destroy({ where: { id: vehicle_id } })
    .then((num) => {
      num === 1
        ? res.send({ message: `Lead id: ${vehicle_id} deleted` })
        : res.status(404).send({ message: "Lead not found" });
    })
    .catch((err) => {
      res.status(500).send({ message: "db error" });
